...
DATABASE_MODES  = [DB_WEB, DB_LOCAL]
...
def checkSinglePassword(l_password, l_current_input_mode, l_current_db_mode, l_cli_local_db_file, l_cli_output_file):
    debugLog("checkSinglePassword(" + l_password + "," + str(l_current_input_mode) + "," + str(l_current_db_mode) + "," + l_cli_local_db_file + "," + l_cli_output_file + ")")
...
# Options
options = "p:f:t:l:o:d:sh"
# Long options
long_options = ["password", "password_file", "text_file", "local_sha1_file", "output_file", "delay", "sha1_format", "help"]
...
if current_operation_mode == IM_SINGLE_PASSOWRD:
    ...
    g_number_of_password_read = 1
    checkSinglePassword(cli_password, cli_input_mode, cli_db_mode, cli_local_db_file, cli_output_file)
    printStats()
